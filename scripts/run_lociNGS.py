...
class MainScreen(Frame):	
	...
	def usedReads(self, X):
		dir = getUsedRawFastaFromBAM(X)
		locShort = X.split(".")
		success = "Used Reads for  [LOCUS: " + locShort[0]+ "]   printed to: "+dir
		self.createWidgets(success, "BLUE")
	...
	def createLocusWindow(self, string):
		...
		label8=Label(frame, text="Coverage_Used").grid(row=0, column=6, padx = 6)
		...
		for locus in locList:
			...
			#	print "X is", X, "string is:", string, "final locus total:", locusTotal	
				...
				button3 = Button(frame, text = usedTotal, command = lambda X=X: self.usedReads(X)).grid(row=1+locList.index(locus), column=6, padx = 6)
	...
	def printAllData(self):
		...
		list1 = getDemoColumnsFromMDB()
		newList1 = []
		printAllDataFile = open('SummaryData.txt', 'w')
		...
		for i in listOfColumns:
			printAllDataFile.write(i)
			printAllDataFile.write("\t")
		printAllDataFile.write("\n")
		...
		for ind in indList:
			...
			for x in cursor:
				...
				#	if listOfColumns.index(i) != 2:
					printAllDataFile.write(str(x[i]))
					printAllDataFile.write("\t")
				printAllDataFile.write("\n")

		locusDataFile = open('LocusAndIndividualData.txt', 'w')
		locusColumns = ["Individual", "Locus","Length","SNPs","Coverage_This_Ind","Number_Inds_Called","Coverage_Total", "Coverage_Used"]
		for cols in locusColumns:
			locusDataFile.write(cols)
			locusDataFile.write("\t")
		locusDataFile.write("\n")
		locList = []
		cursorLoc = loci.find( { "locusName" : { '$exists' : True } } )
#		cursorLoc = loci.find( {"indInFasta": eachOne})
		#for y in cursorLoc:
		#	print y["locusName"]
		#	locList.append(y["locusName"])

		for eachOne in indList:
			#for locus in locList:
			#	label9 = Label(frame, text = locus).grid(row=1+locList.index(locus), column = 0, padx = 6)
			cursor = loci.find( { "locusName" : { '$exists' : True } } )
			for x in cursor:
				fake = {}
				fake = x["individuals"]
				if eachOne in fake:
					...
					usedTotal = 0
					usedReads = {}
					usedReads = x["indInFasta"]
					locusDataFile.write(eachOne)
					locusDataFile.write("\t")
					locusDataFile.write(str(x["locusName"]))
					locusDataFile.write("\t")
					locusDataFile.write(str(x["length"]))
					locusDataFile.write("\t")
					locusDataFile.write(str(x["SNPs"]))
					locusDataFile.write("\t")
					for each in fake:
						if each == eachOne:
							locusDataFile.write(str(fake[each]))
							locusDataFile.write("\t")
						locusTotal = locusTotal + fake[each]
						if each in usedReads:							
							usedTotal = usedTotal + fake[each]
					locusDataFile.write(str(len(x["indInFasta"])))
					locusDataFile.write("\t")
					locusDataFile.write(str(locusTotal))
					locusDataFile.write("\t")
					locusDataFile.write(str(usedTotal))
					locusDataFile.write("\n")
		success = "Successful print of data to "+os.getcwd()+"\n" 
		self.createWidgets(success, "BLUE")
	...
	def casmenu(self):
		...
		casmenu.expmenu.add_separator()
		casmenu.expmenu.add_command(label='SummaryData', command=self.printAllData)
