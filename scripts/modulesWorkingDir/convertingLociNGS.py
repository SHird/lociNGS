...
def getRawFastaFromBAM(ind,locusNumber):
	import pysam
	print ind
	print locusNumber
	file = 'readsFor'+ind+'_'+str(locusNumber)+'.fasta'
		...
		if bam.startswith(ind):
			if bam.endswith(".bam"):
				...
				for reference in samfile.references:
					if reference.endswith('|'+locusNumber):
						bamName = ()
...
def getAllRawFastaFromBAM(locusNumber):
	import pysam
	file = 'allReads_'+locusNumber+'.fasta'
